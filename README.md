...
Autoscaling tests have been performed using min=1 and max=6 instances EKS cluster setup and CPU threshold as scale up/down trigger.
As expected, without any load there was only 1 instance running. After sending 200k interchanges, EKS started to scale up until max 6 instances count has been reached.
Processing has been distributed across all 6 instances and all interchanges have been processed without any error.
When all messages have been processed, EKS has dropped 5 instances leaving the configured min 1 instance running.
